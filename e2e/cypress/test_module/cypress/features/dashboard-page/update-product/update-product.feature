Feature: | Update Product |
    ...
    Background: Before each
        ...
        When I click on the 'first product update icon'

    Scenario: Manager successfully updates one property
    ...
    Scenario: Manager successfully updates all properties
        When I fill in the 'Image URL' field with "http://image.com"
    ...
    Scenario: Server error when manager updates a product
        Given Server returns 500 error when manager tries to update a product
        And I fill in the 'Product Name' field with "test name"
        And I click on the 'Update product' button
        Then I should receive a snackbar with message 'Failed to update product'
