...
jest.mock(
    "@/pages/product-details/components/reserve-button-container/ReserveButtonContainer",
    () => ({
        __esModule: true,
        default: jest.fn(() => (
            <button data-testid="reserve-button">Reserve</button>
        ))
    })
);
...
describe("ProductDetailsContainer", () => {
    ...
    test("hides Favorite and Cart icons for ADMIN", () => {
        ...
        expect(screen.queryByTestId("reserve-button")).not.toBeInTheDocument();
    ...
    test("should display Favorite, Reserve and Buy buttons for USER", () => {
        (useIsAuthSelector as jest.Mock).mockReturnValue(true);
        (useUserRoleSelector as jest.Mock).mockReturnValue(ROLES.USER);
    ...
    test("should dispaly Reserve and Buy buttons but favorite opens auth modal for GUEST", async () => {
        (useIsAuthSelector as jest.Mock).mockReturnValue(false);
